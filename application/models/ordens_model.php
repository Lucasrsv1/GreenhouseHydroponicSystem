<?php
	...
	class Ordens_model extends CI_Model {
		...
		function SelecionarTudo ($empresas_id) {
			...
			$select = self::$local_server->query("SELECT R.id AS 'reles_id', R.rele_pin, R.rele_nome, R.estado, S.ordens_id, S.ordem, S.usuario, S.nome, S.ultima_atualizacao_formatada FROM reles R INNER JOIN (SELECT R.id, OP.id AS 'ordens_id', OP.ordem, C.usuario, CONCAT(C.primeiro_nome, ' ', C.sobrenome) AS 'nome', MAX(DATE_FORMAT(IF (OP.cumprida = FALSE, OP.envio, R.ultima_atualizacao), '%d/%m/%Y %h:%i:%s')) AS 'ultima_atualizacao_formatada' FROM reles R LEFT JOIN ordens_pendentes OP ON OP.reles_id = R.id LEFT JOIN contas C ON OP.contas_id = C.id WHERE R.empresas_id = $empresas_id AND R.em_uso GROUP BY 1) S ON S.id = R.id ORDER BY R.rele_nome ASC, 9 DESC");
		...
		function Cancelar ($id, $empresas_id) {
			if (!self::$local_server)
				return 'errorS';
			
			self::$local_server->where(array('id' => $id, 'empresas_id' => $empresas_id, 'cumprida' => false));
